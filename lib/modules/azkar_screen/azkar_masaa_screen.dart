...
class AzkarMasaaScreen extends StatefulWidget {
  @override
  State<AzkarMasaaScreen> createState() => _AzkarMasaaScreenState();
}

class _AzkarMasaaScreenState extends State<AzkarMasaaScreen> {
  List<Zekr> azkar = [
    Zekr(
        count: 1,
        text:
    ...
    Zekr(
        count: 4,
        text:
            "للَّهُمَّ إِنِّي أَمْسَيتُ أُشْهِدُكَ، وَأُشْهِدُ حَمَلَةَ عَرْشِكَ، وَمَلاَئِكَتِكَ، وَجَمِيعَ خَلْقِكَ، أَنَّكَ أَنْتَ اللَّهُ لَا إِلَهَ إِلاَّ أَنْتَ وَحْدَكَ لاَ شَرِيكَ لَكَ، وَأَنَّ مُحَمَّداً عَبْدُكَ وَرَسُولُكَ"),
    Zekr(
        count: 1,
        text:
    ...
    Zekr(
        count: 1,
        text:
            "اللهم إني أعوذ بك من البَرَصِ، والجُنُونِ، والجُذَامِ، وَسَيِّئِ الأسْقَامِ"),
    Zekr(
        count: 10,
        text: "اللَّهُمَّ صَلِّ وَسَلِّمْ عَلَى نَبَيِّنَا مُحَمَّدٍ"),
  ];
  callback(int count, index) {
    setState(() {
      azkar[index].count = --count;
    ...
    return Scaffold(
      ...
      body: ConditionalBuilder(
        builder: (context) => ListView.builder(
          itemBuilder: (context, index) => BuildAzkar(
            count: azkar[index].count,
            zekr: azkar[index].text,
            callback: () => callback(azkar[index].count, index),
          ),
